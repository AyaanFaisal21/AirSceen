...
Targets spawn sporadically, no faster than every two seconds, no slower than every ten seconds while the overlay is below its cap, and at most four are visible at once. Pinching on a target pops it, and a fast index-finger slash through a target slices it. Slow movement onto a target does not count as a slice.
